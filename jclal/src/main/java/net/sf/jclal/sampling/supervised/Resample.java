...
public class Resample extends AbstractSampling {
    ...
    public void createSubsampleWithReplacement(WekaDataset dataSet,
        ...
        for (int i = 0; i < sampleSize; i++) {
            ...
            if (getRandgen().uniform(0, 1) < biasToUniformClass) {
            	//如果符合此分支，就随机挑选出来一个类cIndex，然后把这个类中挑选出来一个数据，把这个数据的index挑出来
            ...
            } else {//ght默认的是随机抽取sampleSize个样本，而如果指定了biasToUniformClass，
            	//就会按照标准正态分布来抽样，configure中对biasToUniformClass制定了一个数值
    ...
    public void sampling(IDataset dataSet) {
        ...
        //origSize是原始数据集大小，sampleSize是需要抽样的数量（打标签的数据的数量）
        int origSize = wekaDataSet.getNumInstances();
        int sampleSize = (int) (origSize * getPercentageInstancesToLabelled() / 100);

        // Subsample that takes class distribution into consideration二级分类时考虑累的分布
        // Sort according to class attribute.按照类属性排序
        wekaDataSet.getDataset().sort(wekaDataSet.getClassIndex());
        //存放每个类值得初始数据的编号的数组是classIndices
        ...
        for (int i = 0; i < dataSet.getNumInstances(); i++) {
            ...
            //如果current的类别属性缺失了，那么从当前的currentClass开始，后面的类都赋值为i
