...
public class TestNXN {

	private static boolean Iman, Nemenyi, Holm, Scha, Berg; // post-hoc methods
															// to apply
	private static final int MAX_ALGORITHMS = 9;

	private static HashMap<String, double[]> adjustedPValues;
	 ...
	 *            </p>
	 */
	public static void doMultiple(double data[][], String algorithms[], boolean print) {
	 ...
	 *
	 * @param code
	 *            A value to codify which post-hoc methods apply
	 * @param results
	...
	private static String runMultiple(double[][] results, String algorithmName[]) {
		...
		for (i = 0; i < algorithmName.length; i++) {
			out.append((String) algorithmName[i] + " & " + nf4.format(Rj[i]) + "\\\\\n");
		}
		...
		termino2 = (double) algorithmName.length * ((double) algorithmName.length + 1)
				* ((double) algorithmName.length + 1) / (4.0);
		for (i = 0; i < algorithmName.length; i++) {
			sumatoria += Rj[i] * Rj[i];
		}
		...
		out.append("P-value computed by Friedman Test: " + pFriedman + ".\\newline\n\n");
		...
		if (Iman) {
			...
			out.append(
					"Iman and Davenport statistic considering reduction performance (distributed according to F-distribution with "
							+ (algorithmName.length - 1) + " and " + (algorithmName.length - 1) * (nDatasets - 1)
							+ " degrees of freedom: " + nf6.format(iman) + ".\n\n");

			double pIman;

			pIman = FishF(iman, (algorithmName.length - 1), (algorithmName.length - 1) * (nDatasets - 1));
			out.append("P-value computed by Iman and Daveport Test: " + pIman + ".\\newline\n\n");
		}

		termino3 = Math
				.sqrt((double) algorithmName.length * ((double) algorithmName.length + 1) / (6.0 * (double) nDatasets));

		out.append("\n\n\\pagebreak\n\n");

		/************ NxN COMPARISON **************/

		out.append("\\section{Post hoc comparisons}");
		out.append(
		...
		/*
		 * Computing the logically related hypotheses tests (Shaffer and
		 * Bergmann-Hommel)
		 */
		...
		out.append("\\subsection{P-values for $\\alpha=0.05$}\n\n");
		...
		for (i = 0; i < order.length; i++) {
			out.append((order.length - i) + "&" + ordenAlgoritmos[order[i]] + "&"
					+ nf6.format(Math.abs((ordenRankings[order[i]]) / SE)) + "&" + nf6.format(Pi[i]));
			if (Holm) {
				out.append("&" + nf6.format(ALPHAiHolm[i]));
			}
			if (Scha) {
				out.append("&" + nf6.format(ALPHAiShaffer[i]));
			}
			out.append("\\\\\n");
		}
		...
		if (Nemenyi) {
			out.append("Nemenyi's procedure rejects those hypotheses that have an unadjusted p-value $\\le"
		...
		if (Scha) {
			...
			for (i = 0; i < order.length && !parar; i++) {
				if (Pi[i] <= ALPHAiShaffer[i]) {
					out.append("Shaffer's procedure rejects those hypotheses that have an unadjusted p-value $\\le"
		...
		/*
		 * For Bergmann-Hommel's procedure, 9 algorithms could suppose intense
		 * computation
		 */
		if ((algorithmName.length <= MAX_ALGORITHMS) && (Berg)) {
			...
			for (i = 0; i < exhaustiveI.size(); i++) {
				minPi = 2
						* CDF_Normal
								.normp((-1)
										* Math.abs(Rj[((Relation) ((Vector<Relation>) exhaustiveI.elementAt(i))
												.elementAt(0)).i]
										- Rj[((Relation) ((Vector<Relation>) exhaustiveI.elementAt(i)).elementAt(0)).j])
						/ SE);
				...
				for (j = 1; j < ((Vector<Relation>) exhaustiveI.elementAt(i)).size(); j++) {
					tmpPi = 2
							* CDF_Normal.normp((-1)
									* Math.abs(Rj[((Relation) ((Vector<Relation>) exhaustiveI.elementAt(i))
											.elementAt(j)).i]
									- Rj[((Relation) ((Vector<Relation>) exhaustiveI.elementAt(i)).elementAt(j)).j])
							/ SE);
					if (tmpPi < minPi) {
						minPi = tmpPi;
					}
				...
				if (minPi > (0.05 / ((double) ((Vector<Relation>) exhaustiveI.elementAt(i)).size()))) {
					for (j = 0; j < ((Vector<Relation>) exhaustiveI.elementAt(i)).size(); j++) {
						cuadro[((Relation) ((Vector<Relation>) exhaustiveI.elementAt(i))
								.elementAt(j)).i][((Relation) ((Vector<Relation>) exhaustiveI.elementAt(i))
										.elementAt(j)).j] = true;
					}
				}
			}
			...
			if (Berg) {
				...
				cad.append("Bergmann's procedure rejects these hypotheses:\n\n");
				cad.append("\\begin{itemize}\n\n");

				counter = 0;
				for (i = 0; i < cuadro.length; i++) {
					for (j = i + 1; j < cuadro.length; j++) {
						if (cuadro[i][j] == false) {
							cad.append("\\item " + algorithmName[i] + " vs. " + algorithmName[j] + "\n\n");
							counter++;
						}
					}
				}
				cad.append("\\end{itemize}\n\n");

				if (counter > 0) {
					out.append(cad);
				} else {
					out.append("Bergmann's procedure does not reject any hypotheses.\n\n");
				}
			}
		}

		out.append("\\pagebreak\n\n");
		out.append("\\subsection{P-values for $\\alpha=0.10$}\n\n");
		...
		out.append("\\subsection{Adjusted p-values}\n\n");
			...
			if (algorithmName.length <= MAX_ALGORITHMS) {
				...
				for (j = 0; j < exhaustiveI.size(); j++) {
					if (exhaustiveI.elementAt(j).toString().contains(parejitas[order[i]].toString())) {
						minPi = 2
								* CDF_Normal.normp((-1)
										* Math.abs(Rj[((Relation) ((Vector<Relation>) exhaustiveI.elementAt(j))
												.elementAt(0)).i]
										- Rj[((Relation) ((Vector<Relation>) exhaustiveI.elementAt(j)).elementAt(0)).j])
								/ SE);
						for (k = 1; k < ((Vector<Relation>) exhaustiveI.elementAt(j)).size(); k++) {
							tmpPi = 2 * CDF_Normal.normp((-1)
									* Math.abs(Rj[((Relation) ((Vector<Relation>) exhaustiveI.elementAt(j))
											.elementAt(k)).i]
									- Rj[((Relation) ((Vector<Relation>) exhaustiveI.elementAt(j)).elementAt(k)).j])
									/ SE);
							if (tmpPi < minPi) {
								minPi = tmpPi;
							}
		...
		adjustedPValues = new HashMap<String, double[]>();

		for (i = 0; i < Pi.length; i++) {

			String hypo = algorithmName[parejitas[order[i]].i] + "-" + algorithmName[parejitas[order[i]].j];

			adjustedPValues.put(hypo, adjustedP[i]);

			out.append((i + 1) + "&" + algorithmName[parejitas[order[i]].i] + " vs ."
					+ algorithmName[parejitas[order[i]].j] + "&" + nf6.format(Pi[i]));
			if (Nemenyi) {
	 ...
	 *
	 * @param indices
	 *            A verctos of indexes.
	 *
	 * @return A vector with vectors containing all the possible relations
	 *         between the indexes
	 */
	...
	public static Vector<Vector<Relation>> obtainExhaustive(Vector<Integer> indices) {
		...
		for (i = 0; i < indices.size(); i++) {
			for (j = i + 1; j < indices.size(); j++) {
				set.addElement(new Relation(((Integer) indices.elementAt(i)).intValue(),
		...
		for (i = 0; i < result.size(); i++) {
			for (j = i + 1; j < result.size(); j++) {
				if (((Vector<Relation>) result.elementAt(i)).toString()
	...
	private static String header() {
		...
		output.append("\\title{Output tables for the test of Multiple comparisons.}\n");
	...
	public static double[] getAdjustedPValues(String hypo) {
		return adjustedPValues.get(hypo);
	}
