...
public class UtilsSwing {
	...
	public static double validateValue(String text, Component x) throws NumberFormatException {
		...
		try {
			...
			JOptionPane.showMessageDialog(x, "Error parsing to number in field: " + text, "Error message",
	...
	public static boolean exist(JComboBox x) {
		if (x.getSelectedIndex() == -1) {
			JOptionPane.showMessageDialog(x, "Empty list.", "Message", JOptionPane.ERROR_MESSAGE);
			return false;
		}
		return true;
	}
